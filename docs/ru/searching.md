...
* [Команды вида `CALL *`](#команды-вида-call-)
...
### Именованный параметр вместо значения

В выражение именованный параметр пишется как есть (`where('country=:country')`,
`select(['IN(color, :black) as f'])`). На месте значения его запрашивают явно:
...
Обычная строка параметром не считается, как бы она ни выглядела: `':30'` — это введённое
пользователем время, и значение экранируется независимо от своего вида. (До версии 2.0
значение, подходящее под `:слово`, подставлялось в SQL как плейсхолдер, и запрос падал
с ошибкой *Invalid parameter number*, как только это значение действительно было данными.)
...
Строки JOIN отдаются **обычным списком**, а не словарём по id документа, как у простого
SELECT: JOIN возвращает по строке на пару, поэтому id левой таблицы повторяется, как только
связь становится «один ко многим».
...
KNN — это *k-nearest neighbours*, «k ближайших соседей»: запрос спрашивает не строки, где
колонка чему-то равна, а `k` строк, чей вектор ближе всего к заданному.

Вектор здесь — это набор чисел, описывающих объект; обычно его выдаёт эмбеддинг-модель,
превращающая текст или картинку, скажем, в 384 числа. Близкие по смыслу объекты получают
близкие наборы чисел — именно поэтому такой поиск называют семантическим: `match('яблоко')`
найдёт документы с этим словом, а KNN по эмбеддингам найдёт и те, что про яблоки, но самого
слова не содержат.

Колонка типа `float_vector` хранит такой вектор, размерность задаётся схемой:
...
Мера близости выбирается при объявлении колонки:

| Метрика | Что измеряет | Обычно для |
|---|---|---|
| `l2` (по умолчанию) | евклидово расстояние | векторов сопоставимой длины |
| `cosine` | угол между векторами, без учёта их длины | текстовых эмбеддингов |
| `ip` | скалярное произведение | моделей, обученных под него |

Расстояние возвращается в строке как `_knn_dist` — так же, как вес полнотекстового запроса
приходит в `_score`. К выборке `SELECT *` сервер добавляет его сам. При `l2` меньшее расстояние
означает более близкое совпадение: точное попадание даёт почти ноль.
...
предыдущий. Возможность требует, чтобы сервер загрузил KNN-библиотеку: без неё таблицу с
колонкой `float_vector` не получится даже создать — см.
[что требует большего, чем один сервер](../../README.ru.md#что-требует-большего-чем-один-сервер).
